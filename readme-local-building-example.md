...
Let's say we wan to refresh the image `toanlk/ubuntu-vnc-xfce-g3:latest`.
...
The result image tag will get the suffix `_stage_xfce`, e.g. `toanlk/headless-ubuntu-g3:latest_stage_xfce`.
...
For example, the `README` file for the repository `toanlk/ubuntu-vnc-xfce-g3` can be generated by the following command:
